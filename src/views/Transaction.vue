<template>
  <div class="container">
    <div class="row">
      <div class="col">
        <div>Machine : {{head.machine.name}} ({{head.machine.code}})</div>
        <div>Product : {{head.product.name}} ({{head.product.code}})</div>
      </div>
    </div>
    <table class="table table-condensed">
      ...
      <tbody>
        <tr v-for="item in list" :key="item.id">
          <td>{{ item.datetime.substr(0, 19).replace("T", " ") }}</td>
          <td>{{ item.add }}</td>
          <td>{{ item.sub }}</td>
          <td>{{ item.remain }}</td>
          <td>{{ item.diff }}</td>
    ...
    <b-modal id="modal-1" title="Transaction" hide-footer>
      <div class="row">
        <div class="col-12">
...
export default {
  data() {
    return {
      head: {machine:{name:""},product:{name:""}},
      perPageOption: [
        { text: "5 rows", value: 5 },
  ...
  methods: {
    ...
    getList() {
      fetch(_config.apiDomain + "/stock/" + this.$route.params.id, {
        method: "GET",
        headers: {
    ...
    onDelete(id) {
      fetch(_config.apiDomain + "/transaction/" + id, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + localStorage.getItem("access_token"),
